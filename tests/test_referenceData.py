...
class TestReferenceData(TestCase):
    def setUp(self) -> None:
        self.odm_test_file = get_data_file_path("test_referece_data_01.xml")

    def test_reference_data_to_xml(self):
        rd = []
        rd.append(ODM.ReferenceData(
            StudyOID="P2006-101", MetaDataVersionOID="101.01"))
        rd[0].ItemGroupData.append(ODM.ItemGroupData(
            ItemGroupOID="IG.REFSAMP", ItemGroupRepeatKey="1", TransactionType="Insert"))
        rd[0].ItemGroupData[0].ItemData.append(
            ODM.ItemData(ItemOID="IT.REF1", Value="1"))
        rd[0].ItemGroupData[0].ItemData.append(ODM.ItemData(
            ItemOID="IT.REF2", Value="Sample reference data line 1"))
        rd[0].ItemGroupData.append(ODM.ItemGroupData(
            ItemGroupOID="IG.REFSAMP", ItemGroupRepeatKey="2", TransactionType="Insert"))
        rd[0].ItemGroupData[1].ItemData.append(
            ODM.ItemData(ItemOID="IT.REF1", Value="2"))
        rd[0].ItemGroupData[1].ItemData.append(ODM.ItemData(
            ItemOID="IT.REF2", Value="Sample reference data line 2"))
    ...
    def test_audit_records(self):
        ...
        self.assertEqual(
            rd[0].AuditRecords[0].AuditRecord[0].LocationRef.LocationOID,
            "SITE.102")
    ...
    def test_signatures(self):
        ...
        self.assertEqual(
            rd[0].Signatures[0].Signature[0].ID, "SIG.001.USER.002")
        self.assertEqual(
    ...
    def test_annotations(self):
        annotation = ODM.Annotation(
            SeqNum="1", TransactionType="Insert", ID="ANN999")
        ...
        self.assertEqual(
            rd[0].Annotations[0].Annotation[0].Flag[0].FlagValue.CodeListOID, "CL.FLAGVALUE")
    ...
    def create_root(self):
        ...
        """
        root = {"FileOID": "ODM.TEST.RD.001", "Granularity": "Metadata",
                "AsOfDateTime": self.set_datetime(),
                "CreationDateTime": self.set_datetime(),
